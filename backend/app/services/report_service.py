...
def exportar_excel(resultados: list[dict], errores: list[dict], output_dir: str) -> tuple[str, pd.DataFrame]:
    os.makedirs(output_dir, exist_ok=True)
    fecha = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
    archivo_salida = os.path.join(output_dir, f"reporte_procesos_{fecha}.xlsx")
