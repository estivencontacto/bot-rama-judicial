...
def _buscar_driver_cacheado(nombre_driver: str) -> Optional[str]:
    """Busca drivers ya descargados por Selenium Manager para evitar nuevas descargas."""
    ...
    for root, _, files in os.walk(cache_dir):
        if nombre_driver in files:
            candidatos.append(os.path.join(root, nombre_driver))
    if not candidatos:
        return None
...
def configurar_driver(headless: bool | None = None):
    ...
    if settings.selenium_browser.lower() == "chrome":
        ...
        driver_path = _buscar_driver_cacheado("chromedriver.exe")
        ...
        driver_path = _buscar_driver_cacheado("msedgedriver.exe")
