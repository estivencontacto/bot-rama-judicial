...
from backend.app.services.notification_service import (
    construir_mensaje_actuacion_nueva,
...
def _upsert_proceso(db: Session, radicado: Radicado, datos: dict) -> tuple[Proceso, list[dict], bool]:
    """Crea/actualiza proceso y retorna solo actuaciones nuevas para alertar."""
    ...
    for item in actuaciones:
        ...
        if not es_primer_registro:
            nuevas_actuaciones.append(item)

    if tiene_novedad and not actuaciones:
        db.flush()
        item = {
            "Fecha": str(proceso.fecha_ultima_actuacion or ""),
            "Actuacion": "Proceso registrado" if es_primer_registro else "Nueva actuacion detectada",
            "Anotacion": datos.get("Partes") or "Cambio detectado por comparacion de estado",
        }
        db.add(
            Actuacion(
                proceso_id=proceso.id,
                fecha=proceso.fecha_ultima_actuacion,
                titulo=item["Actuacion"],
                descripcion=item["Anotacion"],
                raw_data=datos,
            )
        )
        if not es_primer_registro:
    ...
    return proceso, nuevas_actuaciones, es_primer_registro
    ...
    try:
        ...
        for radicado in radicados:
            try:
                ...
                datos["Cliente"] = radicado.etiqueta or "Sin cliente"
                resultados.append(datos)
                _, nuevas_actuaciones, es_primer_registro = _upsert_proceso(db, radicado, datos)
                ...
                if nuevas_actuaciones:
                    ...
                    consulta.total_novedades += len(nuevas_actuaciones)
                db.commit()

                if nuevas_actuaciones:
                    for actuacion in nuevas_actuaciones:
                        notificar_usuario_telegram_seguro(
                            db,
                            usuario.id,
                            construir_mensaje_actuacion_nueva(datos, actuacion),
                        )
                elif es_primer_registro:
                    consulta.ultimo_mensaje = f"Radicado {radicado.numero} registrado como linea base."
                    db.commit()
...
def ejecutar_consulta_background(consulta_id: int, usuario_id: int, numeros: list[str] | None = None, etiqueta: str | None = None) -> None:
    ...
    try:
        ...
        if not usuario or not consulta:
            return
        ejecutar_consulta_sincrona(db=db, usuario=usuario, numeros=numeros, consulta=consulta, etiqueta=etiqueta)
    except Exception as exc:
...
def lanzar_consulta_background(consulta_id: int, usuario_id: int, numeros: list[str] | None = None, etiqueta: str | None = None) -> None:
    """Encola la consulta en Redis/RQ o proceso local separado segun configuracion."""
    settings = get_settings()
    if not settings.redis_url:
        os.makedirs("output", exist_ok=True)
        log_file = open(os.path.join("output", f"consulta_{consulta_id}.log"), "a", encoding="utf-8")
        subprocess.Popen(
            [
                sys.executable,
                "-m",
                "backend.app.workers.local_consultation_runner",
                str(consulta_id),
                str(usuario_id),
                json.dumps(numeros or []),
                etiqueta or "",
            ],
            cwd=os.getcwd(),
            stdout=log_file,
        ...
        return
    enqueue_job(ejecutar_consulta_background, consulta_id, usuario_id, numeros, etiqueta)
