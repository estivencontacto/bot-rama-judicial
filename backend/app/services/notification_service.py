...
def construir_mensaje_actuacion_nueva(datos: dict, actuacion: dict) -> str:
    """Mensaje puntual para una actuacion nueva detectada contra el historial guardado."""
    titulo_actuacion = actuacion.get("Actuacion") or actuacion.get("titulo") or "Actuacion sin titulo"
    anotacion = actuacion.get("Anotacion") or actuacion.get("descripcion")
    fecha = actuacion.get("Fecha") or actuacion.get("fecha") or "Sin fecha"
    es_auto = "AUTO" in f"{titulo_actuacion} {anotacion or ''}".upper()
    lineas = [
        SEPARADOR,
        "IMPORTANTE AUTO" if es_auto else "NUEVA ACTUACION DETECTADA",
        "",
        f"Radicado: {datos.get('Radicado', 'Sin radicado')}",
        f"Cliente/carpeta: {datos.get('Cliente') or 'Sin cliente'}",
        f"Juzgado: {datos.get('Juzgado') or 'No identificado'}",
        f"Fecha actuacion: {fecha}",
        f"Actuacion: {titulo_actuacion}",
    ]
    if anotacion:
        lineas.append(f"Anotacion: {anotacion}")
    return "\n".join(lineas)
