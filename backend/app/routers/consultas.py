...
def ejecutar_consulta(
    ...
    cliente = (payload.cliente or "").strip() or None
    consulta = crear_consulta_pendiente(db=db, usuario=current_user, numeros=payload.radicados, etiqueta=cliente)
    lanzar_consulta_background(consulta_id=consulta.id, usuario_id=current_user.id, numeros=payload.radicados, etiqueta=cliente)
    registrar_auditoria(
        db,
        current_user,
        "consulta.ejecutada",
