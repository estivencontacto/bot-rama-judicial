...
from backend.app.models import Organizacion, RefreshToken, Usuario, UsuarioRol, utcnow
...
def register(payload: RegisterRequest, db: Session = Depends(get_db)) -> TokenResponse:
    """Registra un usuario inicial con organizacion propia y emite JWT."""
    if len(payload.password) < 8:
        raise HTTPException(status_code=400, detail="La password debe tener minimo 8 caracteres")
    ...
    base_org_name = (payload.organizacion or f"Organizacion de {payload.nombre}").strip()
    org_name = base_org_name
    suffix = 2
    while db.query(Organizacion).filter(Organizacion.nombre == org_name).first():
        org_name = f"{base_org_name} ({suffix})"
        suffix += 1
    organizacion = Organizacion(nombre=org_name)
    db.add(organizacion)
    db.flush()
    ...
    db.commit()
    db.refresh(user)
    registrar_auditoria(db, user, "auth.register", "Usuario registrado desde API publica", "usuario", str(user.id))
